function W=SNN_Train(x,y, W, Ci ,Para )
%SNN_TRAIN Training weights of SNNs
...
% dW =zeros(1,N) ;%Adjustment vector of the weights W

%% Training Weights
trail=1;
while trail<=Ntrai
%     [out] = SNN_Forward(x,W,Para);
%     out=W*Ci';  %Equation (9.1)
%     e=y-out;
%     bete=e*Lr;
    e=y-W*Ci';
    bete=e*Lr;
    if trail==1 % The object initialization of training error for each data
        Ek=abs(e*epsilon);            
    end
    
    if abs(e)>Ek
        for n = 1:1:N
    %         dW(n)=bete*Ci(n); %Equation (9.2)
    %         W(n)=W(n)+dW(n);
            W(n)=W(n)+bete*Ci(n);
        end %the end of "for n = 1:1:N"
    end     %the end of "abs(e)>Ek"
    
    trail=trail+1;
end         %the end of "trail<=Ntrai"

end         %the end of "function W=SNN_Train"
